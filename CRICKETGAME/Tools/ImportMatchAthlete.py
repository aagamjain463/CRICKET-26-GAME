...
NAME = 'SK_Cricketer_Match'
BASE = DEST + '/SK_Cricketer_KitBase'
SRC = u.Paths.project_dir() + 'ArtSource/Exports/C26_MatchAthlete_v003.fbx'
...
def main():
    ...
    if not kit:
        u.log_error('C26_MATCH_KIT_FAIL import produced no asset')
        return
    ...
    ok_height = abs(height - base_height) <= 42.0
    ...
    if problems:
        u.log_error('C26_MATCH_KIT_FAIL ' + ' | '.join(problems))
    else:
        u.log('C26_MATCH_KIT_COMPLETE %s verified against %s' % (NAME, 'SK_Cricketer_KitBase'))
