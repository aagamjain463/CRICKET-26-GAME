...
# Radial coverage removes the visible rectangular billboard edges at pitch contact.
# This is texture-free and leaves the existing pooled geometry/draw count unchanged.
uv = node(mat, u.MaterialExpressionTextureCoordinate)
center = node(mat, u.MaterialExpressionSubtract, const_b=.5)
link(uv, '', center, 'A')
radius2 = node(mat, u.MaterialExpressionDotProduct)
link(center, '', radius2, 'A')
link(center, '', radius2, 'B')
scaled = node(mat, u.MaterialExpressionMultiply, const_b=4.0)
link(radius2, '', scaled, 'A')
inverse = node(mat, u.MaterialExpressionOneMinus)
link(scaled, '', inverse, '')
clamped = node(mat, u.MaterialExpressionSaturate)
link(inverse, '', clamped, '')
soft = node(mat, u.MaterialExpressionMultiply)
link(clamped, '', soft, 'A')
link(clamped, '', soft, 'B')
coverage = node(mat, u.MaterialExpressionMultiply)
link(alpha, '', coverage, 'A')
link(soft, '', coverage, 'B')
output(coverage, '', u.MaterialProperty.MP_OPACITY)
