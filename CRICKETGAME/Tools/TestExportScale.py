...
# Apply rotation so Z is up in world space
...
# Target height is 1.82 meters (182 cm in UE)
scale_to_m = 1.82 / h

for v in m.data.vertices:
    # Rotate by 90 around Z so -Y (facing) becomes +X:
...
bpy.ops.export_scene.fbx(
    filepath=out_fbx,
    use_selection=True,
    global_scale=100.0, # 1.82m -> 182cm
    apply_unit_scale=False,
    axis_forward='X',
    axis_up='Z'
)
print("Exported scale test FBX")
