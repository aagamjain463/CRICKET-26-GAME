...
void AC26PlayerController::EndGesture(int Index,FVector2D P)
{
    ...
    else
    {
        ...
        if(M->PlayerBatting()&&(M->Phase==EC26Phase::Delivery||M->Phase==EC26Phase::RunUp))
        {
            ...
            // The camera is the bowling camera, so screen-right is the batter's leg
            // side and screen-down is the bowler's half - in front of the batter.
            // X negated, Y kept: exactly as in AimAngleFromPull.
