...
def _bat_hands(top, drop=(-5.5, -2.5, -8.5)):
    """Two hands on one handle. The top hand is placed by reach; the bottom hand is offset down the
    handle from it, so the grip stays intact no matter where the stroke carries the hands."""
    return {'hand_l': top, 'hand_r': OFF('hand_l', *drop), 'grip_l': 'bat', 'grip_r': 'bat'}
...
BATTER_RUN_R = _locomotion(reach=30, lift=24, drop=5.0, lean=15,
                           ...
                           bat=(R(.68, .30, .46, -.84), (-4.0, -8.0, -9.0)), cycle=16)
...
# Backlift is shared: hands lift up and back over the off stump, front shoulder drops, weight
# settles on the back foot. Every front-foot stroke grows out of this.
_BACKLIFT = dict(BATTER_READY_R, **{
    'pelvis': (6, 0, -44, 0, -2, -11), 'spine': (12, 0, -11), 'chest': (2, -6, -16),
    'neck': (-10, 0, 32), 'head': (-4, 0, 28),
    'foot_l': (1, 12, GROUND + 2), 'foot_r': (-3, -15, GROUND),
    'elbow_l': (30, -30, 126), 'elbow_r': (-10, -40, 118),
    **_bat_hands(R(0.62, -0.14, -0.52, 0.32)),   # hands lift up and back over the off stump
})
...
COVER_DRIVE = _shot(
    ...
    stride=dict(_BACKLIFT, **{
        ...
        **_bat_hands(R(0.60, -0.22, -0.44, 0.18)),
    }),
    plant=dict(_BACKLIFT, **{
        ...
        **_bat_hands(R(0.70, -0.34, 0.46, -0.82)),
    }),
    ...
    contact=dict(BATTER_READY_R, **{
        ...
        'elbow_l': (16, 18, 128), 'elbow_r': (-22, -8, 104),
        **_bat_hands(R(0.90, -0.32, 0.86, -0.40)),
    }),
...
STRAIGHT_DRIVE = _shot(
    ...
    stride=dict(_BACKLIFT, **{
        'pelvis': (12, 0, -40, 0, 7, -16), 'spine': (17, 0, -11),
        'foot_l': (2, 31, GROUND), **_bat_hands(R(0.60, -0.18, -0.46, 0.20)),
    }),
    plant=dict(_BACKLIFT, **{
        'pelvis': (16, 0, -34, 0, 10, -19), 'spine': (20, 0, -2), 'neck': (-14, 0, 24),
        'foot_l': (2, 33, GROUND), 'foot_r': (-4, -16, GROUND + 3),
        **_bat_hands(R(0.70, -0.28, 0.50, -0.82)),
    }),
    contact=dict(BATTER_READY_R, **{
        'pelvis': (18, 0, -12, 0, 11, -18), 'spine': (21, 0, 14), 'chest': (8, 0, 10),
        'neck': (-17, 0, 8), 'head': (-11, 0, 6),
        'foot_l': (2, 33, GROUND), 'foot_r': (-5, -17, GROUND + 5),
        'elbow_l': (18, 16, 130), 'elbow_r': (-20, -6, 106),
        **_bat_hands(R(0.90, -0.10, 0.92, -0.38)),
    }),
    through=dict(BATTER_READY_R, **{
        'pelvis': (15, 0, 14, 0, 10, -15), 'spine': (14, 0, 24), 'chest': (4, 2, 18),
        'neck': (-12, 0, -6), 'foot_l': (2, 33, GROUND), 'foot_r': (-7, -15, GROUND + 10),
        'elbow_l': (8, 2, 146), 'elbow_r': (-26, -14, 126),
        **_bat_hands(R(0.92, -0.14, 0.34, 0.93)),
    }),
    recover=dict(BATTER_READY_R, **{
        'pelvis': (10, 0, -42, 0, 5, -14), 'spine': (14, 0, -8),
        'foot_l': (2, 24, GROUND), **_bat_hands(R(0.78, -0.23, 0.22, -0.95)),
    }),
)
...
ON_DRIVE = _shot(
    ...
    contact=dict(BATTER_READY_R, **{
        ...
        'elbow_l': (24, 14, 126), 'elbow_r': (-14, -4, 104),
        **_bat_hands(R(0.90, 0.20, 0.90, -0.38)),
    }),
    through=dict(BATTER_READY_R, **{
        'pelvis': (13, 0, 24, 4, 9, -15), 'spine': (13, 0, 30), 'chest': (4, 6, 22),
        'neck': (-10, 0, -14), 'foot_l': (11, 30, GROUND), 'foot_r': (-8, -14, GROUND + 10),
        'elbow_l': (14, 0, 140), 'elbow_r': (-20, -14, 124),
        **_bat_hands(R(0.92, 0.36, 0.32, 0.88)),
    }),
    recover=dict(BATTER_READY_R, **{
        'pelvis': (10, 0, -38, 2, 5, -14), 'spine': (14, 0, -4),
        'foot_l': (7, 22, GROUND), **_bat_hands(R(0.78, -0.17, 0.21, -0.96)),
    }),
)
...
SWEEP = _shot(
    ...
    contact=dict(BATTER_READY_R, **{
        ...
        'knee_r': (-15, -24, 2),
        'elbow_l': (26, 14, 96), 'elbow_r': (-16, -4, 78),
        **_bat_hands(R(0.78, -0.08, 0.34, -0.94)),
    }),
    through=dict(BATTER_READY_R, **{
        ...
        'foot_l': (-6, 33, GROUND), 'foot_r': (-15, -9, GROUND + 13),
        'knee_r': (-15, -24, 2),
        'elbow_l': (34, -6, 104), 'elbow_r': (-10, -22, 88),
        **_bat_hands(R(0.78, 0.09, 0.31, -0.95)),
    }),
    recover=dict(BATTER_READY_R, **{
        'pelvis': (18, 0, -34, -2, 4, -30), 'spine': (18, 0, 0), 'neck': (-14, 0, 20),
        'foot_l': (-4, 26, GROUND), 'foot_r': (-12, -12, GROUND + 3),
        **_bat_hands(R(0.78, -0.12, 0.21, -0.97)),
    }),
    contact_frame=31,
)
...
LEG_GLANCE = _shot(
    'LEGGLANCE',
    backlift=dict(_BACKLIFT, **{
        'pelvis': (7, 0, -42, 0, 0, -12), **_bat_hands(R(0.75, -0.12, -0.04, -0.99)),
    }),
    stride=dict(_BACKLIFT, **{
        'pelvis': (10, 0, -48, 1, 3, -14), 'spine': (14, 0, -12),
        'foot_l': (4, 18, GROUND), **_bat_hands(R(0.78, -0.11, 0.09, -0.99)),
    }),
    plant=dict(_BACKLIFT, **{
        'pelvis': (12, 0, -44, 2, 4, -15), 'spine': (15, 0, -8), 'neck': (-12, 0, 28),
        'foot_l': (5, 20, GROUND), **_bat_hands(R(0.78, -0.10, 0.23, -0.97)),
    }),
    contact=dict(BATTER_READY_R, **{
        'pelvis': (13, 0, -36, 2, 5, -15), 'spine': (16, 0, -2), 'chest': (5, 0, -8),
        'neck': (-13, 0, 22), 'head': (-9, 0, 18),
        'foot_l': (5, 20, GROUND), 'foot_r': (-4, -15, GROUND + 1),
        'elbow_l': (28, 0, 108), 'elbow_r': (-12, -20, 92),
        **_bat_hands(R(0.78, -0.06, 0.31, -0.95)),
    }),
    through=dict(BATTER_READY_R, **{
        'pelvis': (12, 0, -24, 2, 4, -15), 'spine': (14, 0, 10), 'chest': (4, 0, 4),
        'neck': (-11, 0, 12), 'foot_l': (5, 20, GROUND), 'foot_r': (-5, -15, GROUND + 2),
        'elbow_l': (32, -12, 104), 'elbow_r': (-8, -28, 94),
        **_bat_hands(R(0.78, 0.01, 0.20, -0.98)),
    }),
    recover=dict(BATTER_READY_R, **{
        'pelvis': (10, 0, -44, 1, 2, -13), **_bat_hands(R(0.78, -0.07, 0.15, -0.99)),
    }),
...
FIELDER_THROW_LOAD = {
    'pelvis': (6, 0, 48, 0, 2, -12),
    'spine': (8, -6, 32),
    'chest': (4, -8, 22),
    'neck': (-10, 0, -32),
    'head': (-4, 0, -22),
    'foot_l': (8, 20, GROUND + 4),
    'foot_r': (-16, -20, GROUND),
    'knee_l': (8, 90, 56),
    'knee_r': (-16, 40, 42),
    'hand_l': R(0.55, -0.10, 0.98, 0.05),
    'hand_r': R(0.62, -0.38, -0.78, 0.48),
    'elbow_l': (38, 10, 132),
    'elbow_r': (-54, -44, 148),
    'grip_l': 'flat',
    'grip_r': 'ball',
...
PICKUP = {
    ...
    'keys': [
        # Frame 0: Natural running approach stride
        (0, RUN[0][1]),
        # Frame 6: Athletic deceleration stride - breaking down momentum, hips lowering
        (6, {'pelvis': (24, 0, 0, 0, 4, -18), 'spine': (26, 0, 0), 'chest': (10, 0, 0),
             'neck': (-24, 0, 0),
             'foot_l': (14, 22, GROUND), 'foot_r': (-16, -14, GROUND),
             'knee_l': (14, 120, 50), 'knee_r': (-16, 80, 48),
             'hand_l': R(.74, .20, .64, -.70), 'hand_r': R(.74, -.18, .64, -.70),
             'elbow_l': (42, -20, 95), 'elbow_r': (-42, -20, 95),
             'grip_l': 'open', 'grip_r': 'open'}),
        # Frame 14: Deep whole-body lowering - knees flexing deeply, center-of-mass sinking
        (14, {'pelvis': (38, 0, -4, 0, 6, -34), 'spine': (36, 0, 2), 'chest': (10, 0, 0),
              'neck': (-36, 0, 0), 'head': (-12, 0, 0),
              'foot_l': (15, 24, GROUND), 'foot_r': (-18, -18, GROUND + 3),
              'knee_l': (15, 130, 42), 'knee_r': (-18, 50, 28),
              'hand_l': R(.92, .12, .54, -.84), 'hand_r': R(.92, -.12, .54, -.84),
              'elbow_l': (38, 20, 58), 'elbow_r': (-38, 20, 58),
              'grip_l': 'flat', 'grip_r': 'ball'}),
        # Frame 20: Clean ground pickup (Event Pickup) - both hands scoop at turf level, stable base
        (20, {'pelvis': (46, 0, -6, 0, 8, -40), 'spine': (38, 0, 0), 'chest': (12, 0, 0),
              'neck': (-40, 0, 0), 'head': (-16, 0, 0),
              'foot_l': (15, 26, GROUND), 'foot_r': (-18, -20, GROUND + 4),
              'knee_l': (15, 130, 40), 'knee_r': (-20, 60, 36),
              'hand_l': R(.95, .08, .52, -.86), 'hand_r': R(.95, -.08, .52, -.86),
              'elbow_l': (40, 22, 60), 'elbow_r': (-40, 22, 60),
              'grip_l': 'flat', 'grip_r': 'ball'}),
        # Frame 28: Dynamic push-off & rise - center of mass ascends, ball brought securely toward core
        (28, {'pelvis': (24, 0, 10, 0, 4, -22), 'spine': (22, -2, 8), 'chest': (8, -2, 4),
              'neck': (-22, 0, -6),
              'foot_l': (15, 22, GROUND), 'foot_r': (-16, -18, GROUND),
              'knee_l': (15, 120, 48), 'knee_r': (-16, 50, 44),
              'hand_l': R(.74, .14, .68, -.70), 'hand_r': R(.74, -.12, .68, -.70),
              'elbow_l': (40, -10, 85), 'elbow_r': (-40, -10, 85),
              'grip_l': 'flat', 'grip_r': 'ball'}),
        # Frame 36: Weight transfer & coiling side-on - back foot loaded, left arm sighting target
        (36, {'pelvis': (10, 0, 38, 0, -2, -14), 'spine': (10, -4, 26), 'chest': (4, -6, 18),
              'neck': (-12, 0, -28), 'head': (-4, 0, -18),
              'foot_l': (10, 18, GROUND + 8), 'foot_r': (-16, -20, GROUND),
              'knee_l': (10, 90, 58), 'knee_r': (-16, 40, 42),
              'hand_l': R(0.58, -0.12, 0.96, -0.10), 'hand_r': R(0.58, -0.34, -0.76, 0.40),
              'elbow_l': (36, 8, 128), 'elbow_r': (-52, -42, 142),
              'grip_l': 'flat', 'grip_r': 'ball'}),
        # Frame 42: Continuous transition pose - seamlessly matches THROW frame 0
        (42, FIELDER_THROW_LOAD),
    ],
}
...
THROW = {
    ...
    'keys': [
        # Frame 0: Continuous start from loaded throw posture (exact match to PICKUP frame 42)
        (0, FIELDER_THROW_LOAD),
        # Frame 8: Crow-hop stride forward - back foot drives, front leg reaches
        (8, {'pelvis': (6, 0, 44, 0, 4, -12), 'spine': (8, -4, 30), 'chest': (4, -6, 20),
             'neck': (-10, 0, -30), 'head': (-4, 0, -20),
             'foot_l': (6, 32, GROUND + 2), 'foot_r': (-16, -20, GROUND + 4),
             'knee_l': (6, 120, 50), 'knee_r': (-16, 40, 42),
             'hand_l': R(0.52, -0.06, 0.98, 0.06), 'hand_r': R(0.64, -0.40, -0.76, 0.50),
             'elbow_l': (38, 6, 134), 'elbow_r': (-54, -42, 150),
             'grip_l': 'flat', 'grip_r': 'ball'}),
        # Frame 16: Front foot firm plant & rotational uncoil - hips turn open, high elbow leads
        (16, {'pelvis': (8, 0, 24, 0, 8, -12), 'spine': (12, -4, 18), 'chest': (8, -6, 12),
              'neck': (-12, 0, -18), 'head': (-6, 0, -12),
              'foot_l': (6, 36, GROUND), 'foot_r': (-16, -22, GROUND + 3),
              'knee_l': (6, 140, 46), 'knee_r': (-16, 36, 42),
              'hand_l': R(0.68, 0.08, 0.35, -0.65), 'hand_r': R(0.72, -0.25, -0.45, 0.82),
              'elbow_l': (40, -8, 124), 'elbow_r': (-48, -36, 168),
              'grip_l': 'flat', 'grip_r': 'ball'}),
        # Frame 26: High overarm release (Event ThrowRelease) - front leg braced, arm whips overhead
        (26, {'pelvis': (12, 0, 8, 0, 12, -10), 'spine': (18, -4, 4), 'chest': (14, -6, 0),
              'neck': (-14, 0, -6), 'head': (-8, 0, -6),
              'foot_l': (6, 36, GROUND), 'foot_r': (-16, -24, GROUND + 12),
              'knee_l': (6, 145, 44), 'knee_r': (-16, 40, 44),
              'hand_l': R(0.78, 0.12, -0.14, -0.98), 'hand_r': R(0.98, 0.10, 0.58, 0.84),
              'elbow_l': (42, -18, 114), 'elbow_r': (-34, -28, 178),
              'grip_l': 'open', 'grip_r': 'open'}),
        # Frame 36: Diagonal follow-through wrap - throwing arm sweeps across chest toward left hip
        (36, {'pelvis': (20, 0, -2, 0, 10, -10), 'spine': (26, 0, -4), 'chest': (14, 0, -2),
              'neck': (-20, 0, 0), 'head': (-10, 0, 0),
              'foot_l': (6, 36, GROUND), 'foot_r': (-10, 10, GROUND + 12),
              'knee_l': (6, 140, 44), 'knee_r': (-10, 60, 52),
              'hand_l': R(0.85, 0.14, -0.40, -0.90), 'hand_r': R(0.86, 0.26, 0.32, -0.88),
              'elbow_l': (44, -24, 118), 'elbow_r': (-18, 16, 96),
              'grip_l': 'open', 'grip_r': 'open'}),
        # Frame 42: Step-through foot plant & momentum absorption - trailing leg grounds to dissipate energy
        (42, {'pelvis': (14, 0, 0, 0, 4, -12), 'spine': (18, 0, 0), 'neck': (-16, 0, 0),
              'foot_l': (8, 28, GROUND), 'foot_r': (-14, 20, GROUND),
              'knee_l': (8, 120, 48), 'knee_r': (-14, 110, 48),
              'hand_l': R(0.78, 0.16, 0.50, -0.82), 'hand_r': R(0.80, -0.16, 0.50, -0.82),
              'elbow_l': (44, -20, 102), 'elbow_r': (-44, -20, 102),
              'grip_l': 'open', 'grip_r': 'open'}),
        # Frame 48: Balanced recovery into athletic fielding stance
        (48, FIELDER_READY),
    ],
}
...
BATTER_CELEBRATE_R = [
    ...
    (10, dict(BATTER_READY_R, **{'pelvis': (4, 0, -30, 0, 0, -8), 'spine': (-4, 0, -6),
                                 'neck': (-12, 0, 16),
                                 **_bat_hands(R(0.33, -0.06, 0.56, -0.83))})),
